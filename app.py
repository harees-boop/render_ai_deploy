...
lang_to_tts_voice = {
    ...
    'nl': 'nl-NL-Wavenet-A', 'en': 'en-US-Wavenet-F', 'en-US': 'en-US-Wavenet-F',
    'fr': 'fr-FR-Wavenet-C', 'fr-FR': 'fr-FR-Wavenet-C',
    'de': 'de-DE-Wavenet-D', 'de-DE': 'de-DE-Wavenet-D',
    'hi': 'hi-IN-Wavenet-B', 'hi-IN': 'hi-IN-Wavenet-B',
    'id': 'id-ID-Wavenet-A',
    'it': 'it-IT-Wavenet-B', 'it-IT': 'it-IT-Wavenet-B',
    'ja': 'ja-JP-Wavenet-C', 'ko': 'ko-KR-Wavenet-A',
    'pl': 'pl-PL-Wavenet-A', 'pl-PL': 'pl-PL-Wavenet-A',
    'pt': 'pt-BR-Wavenet-D', 'pt-BR': 'pt-BR-Wavenet-D',
    'ro': 'ro-RO-Wavenet-A',
    'ru': 'ru-RU-Wavenet-B',
    'es': 'es-ES-Wavenet-A', 'es-ES': 'es-ES-Wavenet-A',
    'ta': 'ta-IN-Wavenet-B', 'te': 'te-IN-Wavenet-A',
    'th': 'th-TH-Wavenet-C',
    'tr': 'tr-TR-Wavenet-A', 'tr-TR': 'tr-TR-Wavenet-A',
    'uk': 'uk-UA-Wavenet-A', 'vi': 'vi-VN-Wavenet-A',
}

LANGUAGE_NAMES = {
    'af': 'Afrikaans', 'ar-EG': 'Arabic (Egypt)', 'bn-BD': 'Bengali (Bangladesh)',
    'nl': 'Dutch', 'en-US': 'English (United States)', 'fr-FR': 'French (France)',
    'de-DE': 'German (Germany)', 'hi-IN': 'Hindi (India)', 'id': 'Indonesian',
    'it': 'Italian', 'ja': 'Japanese', 'ko': 'Korean', 'pl': 'Polish',
    'pt-BR': 'Portuguese (Brazil)', 'ro': 'Romanian', 'ru': 'Russian',
    'es-ES': 'Spanish (Spain)', 'ta': 'Tamil (India)', 'te': 'Telugu (India)',
...
def format_markdown_output(text: str) -> str:
    ...
    for i, line in enumerate(lines):
        stripped = line.strip()
        if (stripped.startswith("#") or re.match(r"^[-*+] ", stripped) or re.match(r"^\d+\.", stripped)):
...
def clean_markdown_to_plain_text(md_text: str) -> str:
    ...
    for line in lines:
        ...
        if heading_match:
            ...
            cleaned_lines.append(content.upper())
...
def extract_text_from_pdf(pdf_path: str) -> str:
    try:
        ...
        if DEBUG: print(f"Error extracting text from PDF '{pdf_path}': {e}")
        return ""
...
def extract_images_from_pdf(pdf_path: str):
    ...
    try:
        ...
        if DEBUG: print(f"Error extracting images from PDF '{pdf_path}': {e}")
        return []
...
def generate_dynamic_system_prompt(syllabus_input: str, subject_input: str, user_name: str, grade_input=None, assistant_name="AI Assistant"):
    ...
    level = {
        "cbse": "school", "icse": "school", "cambridge igcse": "school", "gcse": "school",
        "ib": "school", "a-levels": "college", "ap": "college", "us common core": "school",
        "singapore moe": "school", "chinese national curriculum": "school",
        "japanese mext": "school", "french education": "school",
    }.get(syllabus_key, "appropriate")

    grade_levels = {
        "grade 1": "beginner", "grade 2": "beginner", "grade 3": "elementary",
        "grade 4": "elementary", "grade 5": "intermediate", "grade 6": "intermediate",
        "grade 7": "upper-intermediate", "grade 8": "upper-intermediate",
        "grade 9": "advanced", "grade 10": "advanced", "grade 11": "college-prep",
    ...
    base_prompt = (
        ...
        f"within the {level} syllabus '{syllabus_input}'. You are coaching a student named '{user_name_safe}'.\n\n"
        f"*YOUR TEACHING STYLE:*\n"
        f"1. Speak Naturally. 2. Personal Connection using '{user_name_safe}'. "
        f"3. Check for Understanding. 4. Encourage.\n\n"
    )

    if grade_level:
        base_prompt += f"Strictly tailor explanations to {grade_level} proficiency for {syllabus_input}. "

    base_prompt += (
        "Format with markdown. NEVER use LaTeX or special math syntax. Use standard keyboard symbols. "
        "Avoid restricted topics. Identify as an AI from Hike Learning."
    )
    return base_prompt

def convert_text_to_ssml(text: str) -> str:
    text = text.replace('^2', ' squared ').replace('^3', ' cubed ').replace(' P(A|B) ', ' The probability of A given B ')
...
def synthesize_tts_gemini(text: str, output_filename: str, access_token: str, language_code: str):
    ...
    # --- FIX: Fallback for short codes like 'fr' -> 'fr-FR-Wavenet-C' ---
...
def api_chat():
    ...
    # 1. Save Image (Supports 'image' singular or 'images' plural)
...
def run_chatbot(syllabus, subject, grade, language_code, question, image_paths, pdf_paths, audio_path, user_name):
    ...
    if contains_sensitive_content(combined_text):
        return "Sorry, your input includes sensitive topics.", None
    ...
    if not any(isinstance(c, str) for c in contents) and not audio_path and not image_paths:
         # If really nothing was sent
         return "I didn't receive any content to analyze.", None
    ...
    try:
        ...
        if access_token:
            ...
            return answer_plain, tts_filename 
...
#not worked for image because image and images mismatch in the render with django
...
#     'it': 'it-IT-Wavenet-B', 'ja': 'ja-JP-Wavenet-C', 'ko': 'ko-KR-Wavenet-A',
#     'pl': 'pl-PL-Wavenet-A', 'pt-BR': 'pt-BR-Wavenet-D', 'ro': 'ro-RO-Wavenet-A',
#     'ru': 'ru-RU-Wavenet-B', 'es-ES': 'es-ES-Wavenet-A', 'ta': 'ta-IN-Wavenet-B',
#     'te': 'te-IN-Wavenet-A', 'th': 'th-TH-Wavenet-C', 'tr': 'tr-TR-Wavenet-A',
#     'uk': 'uk-UA-Wavenet-A', 'vi': 'vi-VN-Wavenet-A',
# }
